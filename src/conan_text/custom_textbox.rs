...
///
/// [`Formatter`]: druid::text::format::Formatter
...
impl<T: EditableText + TextStorage> CustomTextBox<T> {
    ...
    pub fn multiline() -> Self {
        let mut this = CustomTextBox::new();
        this.inner
            .set_enabled_scrollbars(druid::scroll_component::ScrollbarsEnabled::Both);
...
impl<T: TextStorage + EditableText> CustomTextBox<T> {
...
impl<T: TextStorage + EditableText> Widget<T> for CustomTextBox<T> {
...
impl<T: TextStorage + EditableText> Default for CustomTextBox<T> {
    fn default() -> Self {
        CustomTextBox::new()
    }
}
