...
impl<T: Data> IconButton<T> {
    ...
    ///
    /// ```
    /// use druid::widget::IconButton;
    ///
    /// let button = IconButton::new("Increment").on_click(|_ctx, data: &mut u32, _env| {
    ///     *data += 1;
    /// });
    /// ```
    ///
    /// [`.on_click`]: #method.on_click
    pub fn new(text: impl Into<LabelText<T>>) -> IconButton<T> {
        IconButton::from_label(Label::new(text))
    }
    ...
    /// ```
    /// use druid::Color;
    /// use druid::widget::{IconButton, Label};
    ///
    /// let button = IconButton::from_label(Label::new("Increment").with_text_color(Color::grey(0.5))).on_click(|_ctx, data: &mut u32, _env| {
    ///     *data += 1;
    /// });
    /// ```
    ///
    ...
    /// [`.on_click`]: #method.on_click
    pub fn from_label(label: Label<T>) -> IconButton<T> {
        IconButton {
            label,
    ...
    /// use druid::Env;
    /// use druid::widget::IconButton;
    /// let button1: IconButton<u32> = IconButton::new(|data: &u32, _: &Env| format!("total is {}", data));
    /// let button2: IconButton<u32> = IconButton::dynamic(|data, _| format!("total is {}", data));
    /// ```
    ///
    /// [`new`]: #method.new
